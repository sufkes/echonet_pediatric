...
def main(dicom_in_dir, datasheet_path, image_out_dir, file_list_out_path, out_color_mode, out_image_type, new_height, new_width, pad, rescale, remove_green_line, split_peaks, save_copy_for_annotation, read_annotations, annotations_in_dir):
    # Load the datasheet.
    base_df = pd.read_csv(datasheet_path)
    ...
    for ii, in_path in enumerate(dicom_in_paths):
        ...
        if read_annotations:
            annotation_name = subject + '.png' # Assume file is named <subject>.png
        ...
        if right != 913:
            warnings.warn('RegionLocationMaxX1 has value other than 913. This may be because part of the VT plot is blacked out. Assuming the right edge of the v-t plot occurs at column 913, and ignoring the value of RegionLocationMaxX1.')
            right = 913
        ...
        # 2023-01-25 SU: We assume that the script has been run previously with --save_copy_for_annotation, the resulting PNG files have been annotated, and we are now rerunning with --read_annotations. We want to check that the size of the annotated PNG file is the same as the size of the extracted v-t plot.
        if read_annotations:
        ...
        for image_num, (pixel_data_ycbcr, annotation) in enumerate(zip(image_list, annotation_list), 1):
            ...
            if split_peaks:
                subject_new = subject+'_'+str(image_num) # each sub-image/peak will be called <subject_name>_<peak #>
            else:
                subject_new = subject
...
if __name__ == '__main__':
    ...
    description = """Extract velocity-time plot from VTI DICOM images, post-process, save to PNG for annotation, read annotations and compute VTI, extract individual VTI peaks, and generate a CSV containing data for each processed PNG to be used in the model."""
